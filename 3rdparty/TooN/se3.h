...
public:
    ...
    // Added by cchoi
    inline SE3& operator=(const Matrix<4>& rhs);
    //// Added by cchoi
    //inline Matrix<4> operator=(const SE3& se3);//d
    //// Added by cchoi
    inline Matrix<3> getRot() { return my_rotation.get_matrix(); };
    inline Vector<3> getTrans() { return my_translation; };
...
// Added by cchoi
inline SE3& SE3::operator=(const Matrix<4>& rhs){
  Matrix<3> M;
  M(0,0) = rhs(0,0);
  M(0,1) = rhs(0,1);
  M(0,2) = rhs(0,2);
  ...
  my_translation[0] = rhs(0,3);
  my_translation[1] = rhs(1,3);
  my_translation[2] = rhs(2,3);
  return *this;
}
//
//// Added by cchoi
//inline Matrix<4> SE3::operator=(const SE3& se3){
//  //Matrix<3> M;
//  //M(0,0) = rhs(0,0);
//  //M(0,1) = rhs(0,1);
//  //M(0,2) = rhs(0,2);
...
//  //my_translation[2] = rhs(2,3);
//    Matrix<4> M;
//    Matrix<3> Rot = se3.my_rotation.get_matrix();
//    Vector<3> Trans = se3.my_translation;
//  //return *this;
//    M(0,0) = Rot(0,0);
//    M(0,1) = Rot(0,1);
//    M(0,2) = Rot(0,2);    
